...
$("#modalAgregarSeries, #modalEditarSerie").draggable({
  handle: ".modal-header"
});
var arrayAlfanumerico = new Array();
...
$("#selecProductoSerie").change(function (event) {
  ...
  let mostrarprod;
  let textalmacen = $("#nuevoAlmacenSerie option:selected").val();
  let idalmacen = textalmacen.substr(0, textalmacen.indexOf('-'));  //extrae el Id del almacen
  let almacen = $("#nuevoAlmacenSerie option:selected").text();
  let numdocto = $("#numdocto").val();
  let idproducto = $("#selecProductoSerie").val();

  //console.log(idalmacen, almacen, numdocto, idproducto)

  const apiUrl = 'ajax/adminseries.ajax.php';
  const parametro1 = 'validdata';
  const parametro2 = idalmacen;
  const parametro3 = numdocto.toString();
  const parametro4 = idproducto;

  const url = `${apiUrl}?op=${parametro1}&idalmacen=${parametro2}&numdocto=${parametro3}&idproducto=${parametro4}`;
  ...
  ajax({
    ...
    (response) => {
      ...
      if (response.response === null || response.response == false) {
        $("#selecProductoSerie").val("");
        $('#msgerror').text('Algún dato es incorrecto. Verifique!!');
    ...
    (error) => {
      ...
      //console.error(error);
      $("#selecProductoSerie").val("");
      $('#msgerror').text('Algún dato es incorrecto. Verifica!!');
      $("#msgerror").removeClass("d-none");
      setTimeout(function () { $("#msgerror").addClass("d-none") }, 2500);
    }
  );
...
$("#agregarProductosSeries").click(function (event) {
  ...
  console.log("#Alma:", idalmacen, "#Prod.", idProducto, "#Serie:", nNumeroSerie, "#Alfanúmerico", nAlfanumerico)
  ...
  let encontrado = arrayAlfanumerico.includes(nAlfanumerico)
  console.log("encontrado:", encontrado)
  if (!encontrado) {
    arrayAlfanumerico.push(nAlfanumerico);
    var fila = '<tr class="filas" id="fila' + cont + '">' +

      '<td><button type="button" class="botonQuitar" onclick="eliminarDetalleDev(' + cont + ')" title="Quitar Concepto">X</button></td>' +

      '<td class="text-center"><input type="hidden" class="prodseries" name="idProducto[]" value="' + idProducto + '">' + idProducto + '</td>' +
    ...
    $('#msgerror').text('Alfanúmerico ya existe. Verifique!!');
    $("#msgerror").removeClass("d-none");
    setTimeout(function () {
      $("#msgerror").addClass("d-none")
    }, 2500);
...
function listarSeries() {
  ...
  let valorradio=($('input:radio[name=statuseries]:checked').val());
  let valoryear=$("input[name='filterdate']").val();
  let arrayFecha = rangodeFecha.split(" ", 3);

 console.log("rangodeFecha:", rangodeFecha, "valorradio:", valorradio, "valoryear:", valoryear)
  
  // console.log("Rango de Fecha:",rangodeFecha);
  if((arrayFecha[0].split("-").length)<3 && (arrayFecha[2].split("-").length)<3) {
    var FechDev1 = moment().format('YYYY-MM-DD');
    var FechDev2 = moment().format('YYYY-MM-DD');
  } else {
    console.log(arrayFecha.length);
  ...
  tabladeSeries = $('#DatatableSeries').dataTable(
    {
      ...
      "lengthMenu": [[20, 35, 50, 100, -1], [20, 35, 50, 100, "Todos"]],
      "language": {
      ...
      "columnDefs": [
        { className: "dt-center", targets: [6, 7, 8, 9, 10, 11] },
      ],
      "ajax":
      {
        url: 'ajax/adminseries.ajax.php?op=listarseries',
        data: { "FechDev1": FechDev1, "FechDev2": FechDev2, "typeview": valorradio, "filteryear": valoryear},
        type: "GET",
        dataType: "json",
        done: function(ev){
...
/*===  ABRE EL MODAL PARA EDITAR SERIES ===*/
$("#modalEditarSerie").on('show.bs.modal', function (event) {
  var button1 = $(event.relatedTarget.dataset) // Button that triggered the modal
  for (var atributo in button1[0]) {
    let valor = button1[0][atributo];
    //console.log(`Atributo: ${atributo}, Valor: ${valor}`);
    $("#" + atributo).val(valor);
  }
  //let nombreont = $(event.relatedTarget.dataset.idnombreont) // Button that triggered the modal
  //console.log(button3.selector)
  //$("input[name='nombredeont']").val(idnombreont);

});
//================= FIN MODAL =================*/
/*=== CUANDO CIERRA EL MODAL VACIAR EL ELEMENTO OBJECT ===*/
$("#modalEditarSerie").on('hidden.bs.modal', () => {
...
/*======================================================================*/
//ENVIAR FORMULARIO PARA GUARDAR DATOS DE ENTRADA
$("body").on("submit", "#formEditaSeries", function (event) {
  event.preventDefault();
  event.stopPropagation();
  //console.log("enviar form");
  let formData = new FormData($("#formEditaSeries")[0]);
  for (var pair of formData.entries()) { console.log(pair[0] + ', ' + pair[1]); }
  //CODIFICAR INSERTAR EN BD
  (async () => {
    await axios({
      method: 'post',
      url: 'ajax/adminseries.ajax.php?op=guardarEditaSerie',
      data: formData,
    })
      .then((res) => {
        //console.log(res);
