...
switch ($_GET["op"]) {
    ...
    case 'listar':
        ...
        foreach ($listarSalidas as $key => $value) {
            $fecha = date('d-m-Y h:i:s', strtotime($value["fecha_transaccion"]));
    ...
    case 'datosBeneficiario':
        ...
        $respuesta = ControladorCtrolDepositos::ctrDatosBeneficiario($tabla, $campo, $valor, $estatus);
    ...
    case 'agregar':
        ...
        if (isset($_POST["idDeUsuario"])) {
            ...
            //if (!preg_match('/^[$\.\,\-\/a-zA-Z0-9\u00E0-\u00FC ]+$/', $_POST["nvoMotivoDeposito"])) {
                if (!preg_match('/^[$\.\,\-\/a-zA-Z0-9\ ]+$/', $_POST["nvoMotivoDeposito"])) {
 ...
 */
            ...
            $patron = '/^[A-Z]{3}$/';       //preg_match que solo acepte 3 letras mayusculas
            if (preg_match($patron, $nvoSucursal)) {
                $respuesta += array('nvoSucursal' => $nvoSucursal, 'result5' => 'texto');
            } else {
                $respuesta += array('nvoSucursal' => $nvoSucursal, 'result5' => 'solo acepta 3 letras mayusculas');
            }
